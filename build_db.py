...
class BrainGraphDB:
    ...
    def import_nodes(
        ...
        if source_path.exists():
            ...
            if source_text_cache is not None and source_file in source_text_cache:
                ...
                try:
                    ...
                    print(
                        f"Warning: Could not read source file {source_file}: {e}",
                        file=sys.stderr,
                    )
        ...
        for node in nodes:
            ...
            if (
                ...
                and "char_start" in node
                and "char_end" in node
            ):
                try:
                    ...
                    print(
                        f"Warning: Could not extract text for chunk {node.get('id')}: {e}",
                        file=sys.stderr,
                    )
            ...
            batch_data.append(
                [
                    ...
                    node.get(
                        "code_language",
                        node.get("language") if node.get("type") == "code" else None,
                    ),
                ]
            )
        ...
        if total_time > 0.5:  # Only print if slow
            ...
            print(
                f"    [Nodes: JSON={json_time:.2f}s, Source={source_time:.2f}s, Prep={prep_time:.2f}s, Insert={insert_time:.2f}s, Total={total_time:.2f}s]",
                file=sys.stderr,
            )
    ...
    def import_embeddings(
        self, parquet_path: Path, source_file: str, target_dim: int = 256
    ):
        """
        ...
        try:
            ...
            for _, row in df.iterrows():
                chunk_id = (
                    row["chunk_idx"]
                    if isinstance(row["chunk_idx"], str)
                    else f"chunk_{row['chunk_idx']:08x}"
                )
            ...
            print(
                f"    Error importing embeddings from {parquet_path}: {e}",
                file=sys.stderr,
            )
            raise
    ...
    def import_directory(self, data_dir: Path):
        ...
        for nodes_file in sorted(nodes_files):
            ...
            edges_file = (
                data_dir / "edges" / month_folder / f"{filename_base}.edges.json"
            )
            embeddings_file = (
                data_dir / "embeddings" / month_folder / f"{filename_base}.parquet"
            )
            ...
            ner_nodes_file = (
                ...
                / "nodes"
                / month_folder
                / f"{filename_base}.ner.nodes.json"
            )
            ner_edges_file = (
                data_dir
                / "ner"
                / "edges"
                / month_folder
                / f"{filename_base}.ner.edges.json"
            )
            ...
            if edges_file.exists():
                ...
                edge_id_offset = self.import_edges(
                    edges_file, source_file, edge_id_offset
                )
            ...
            if ner_edges_file.exists():
                ...
                edge_id_offset = self.import_edges(
                    ner_edges_file, source_file, edge_id_offset
                )
