...
def test_fuzzy_search_finds_typos() -> None:
    ...
    con.execute(
        ...
        ["c1", "u1", "Maschinelles Lernen ist ein Teilgebiet der KI.", "doc1.md"],
    )
...
def test_exact_string_search_is_case_insensitive_by_default() -> None:
    ...
    con.execute(
        "INSERT INTO nodes VALUES (?, ?, 'chunk', ?, NULL, ?)",
        ["c1", "u1", "Deep Learning is fun.", "doc1.md"],
    )
...
def test_semantic_search_orders_by_cosine_similarity() -> None:
    ...
    con.execute(
        "INSERT INTO chunk_embeddings_256d VALUES (?, ?::FLOAT[256])", ["c1", emb1]
    )
    con.execute(
        "INSERT INTO chunk_embeddings_256d VALUES (?, ?::FLOAT[256])", ["c2", emb2]
    )
